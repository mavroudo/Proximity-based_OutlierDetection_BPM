...
last="oursFactorStatistic_trace"
...
for index,outlier in enumerate(outliers):
    ...
    df["recall"]=df["precision"]*df["reported"]/true_outliers[index]
...
x=plot_stats(ranks)
plt.tight_layout()
x.get_figure().savefig("output2/results/prob_distances.eps",format="eps")